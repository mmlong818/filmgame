import type { Project, ValidationIssue, ValidationReport } from '../types/project.ts'
import { nanoid } from 'nanoid'
import { reachableNodeIds } from '../graph.ts'
import { checkVariableRefs } from './varRefs.ts'
...
export function runValidation(project: Project): ValidationReport {
  ...
  // 变量/角色引用检查（期 2，见 lib/validation/varRefs.ts）：全部读引用层 AST。
  // UNRESOLVED / AMBIGUOUS / DANGLING_VARIABLE_REF、UNPARSEABLE_EFFECT、CONDITION_SYNTAX、
  // UNSATISFIABLE_CONDITION、ALL_CHOICES_GATED、UNBOUND_SPEAKER 八个 code 都在那里产出。
  issues.push(...checkVariableRefs(project, safeNodes, endingDefs))
