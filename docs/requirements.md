...
| 项 | 内容 |
|----|------|
| 文档版本 | 1.2（v0.7 立项 FR-18 分支网络规范 / FR-19 定向重构，实施计划见 `docs/plans/2026-08-31-branch-network.md`；1.1 新增 FR-15/16/17、NFR-5） |
| 对应产品版本 | v0.6.0（FR-18/19 为 v0.7 立项项） |
| 日期 | 2026-08-31 |
...
### FR-18 分支网络结构规范（v0.7 立项）

互动影游的常规结构是「分支-汇合网络」，不是"单线推进 + 终章扇出"。结构生成必须满足：

- **中段分支强制**：每章至少 1 个中段分支节点（2-3 条差异路径），非终章分支默认菱形汇合（branch → 路径 → merge 汇回主线）；终章保留变量门控扇出。骨架模板支持三种基本形态：菱形分支-汇合（默认）/ 章内平行路线 / 终章门控扇出，按章位置与预算选择。
- **密度目标**：全片分支节点占比 ≥25%（与校验 LOW_BRANCH_DENSITY 阈值对齐）；预算允许时每章 ≥1 探索节点。
- **变量闭环**：中段分支的选项必须写 variableEffects，终章门控读取这些变量——形成「前期选择 → 后期后果」；每个带条件的分支节点必须保留至少一个无条件保底出口（生成约束与校验 ALL_CHOICES_GATED 对齐）。
- **预算下限联动**：幕节点预算下限 = 4（入口 + branch + 2 路径）；规模方案生成时按此下限分配（体量小则减少幕数而非砍掉分支），杜绝"预算不足 → 自动退化为线性"（v0.6 精简版单线问题的根源，见 `lib/ai/prompts.ts` buildActNodes 预算规则）。
- 修正骨架模板与分支规则文本的 merge 矛盾（模板插入 merge 节点 vs 规则文本"严禁 merge"）。

**v2 修订（依据 `docs/genre-baseline.md` 体验守则，用户确认于 2026-08-31）**：

- **推进节点真选择**（守则 1/2/4）：normal 节点也必须生成 2-3 个对话选项——目标节点相同、variableEffects 与台词语气不同（"主线不变但选择是真的"，RPG 对话树范式）；无 AI 选项节点的「继续」自动补全仅作兜底。
- **每章即死岔口**（守则 3）：非终章每章骨架植入 1-2 个通向 BAD END 小结局的岔口（分支的一条路径 → 1 节点 bad ending，死法必须呼应人物弱点或世界规则——即死是性格测验非随机惩罚；参照《隐形守护者》Gauntlet 与互动短剧「选错→BE→回溯」商业形态）；中途 BE 节点在结局定义导入时自动生成 bad 类型定义。
- **路径真差异**（守则 2）：菱形分支各路径 ≥2 节点；预算不足时优先裁剪推进节点，不得压缩路径长度。
- **并回回响**（守则 4）：merge 后首个节点必须按路线变量写差异化开场台词（骨架 notes 强制）。
- **章末钩子**（守则 7）：每章最后一个非结局节点以悬念/反转/倒计时收束（骨架 notes 强制，适配漫剧 1-5 分钟/集节奏）。

### FR-19 定向重构：按校验 / 导演终审结果修复结构（v0.7 立项）

打通「生成 → 诊断 → 修复」回路的最后一环：

- 新增 AI 动作 `structure:targeted_fix`：输入 = 当前结构摘要 + 本地校验 issues + 导演终审 mustFix；输出 = **节点级补丁**（新增分支/拆分节点/补选项/补 variableEffects/补保底出口/绑定结局定义），而非整体重生成。
- 已填充的对白与场景内容不得被清空；受影响节点标记提示，交由工坊精修。
- 补丁走预览-采纳制（与结构生成一致的 draft 门控），逐项可勾选采纳；应用前自动压撤销快照（FR-15），应用后自动重跑本地校验并展示通过率前后对比。
- 入口：结构页辅助区「按校验结果定向重构」；校验页问题清单区提供跳转。

### FR-20 AI 设置：模型自动发现与连接测试（v0.7）

- BYOK provider（anthropic / openai / gemini / custom）输入 API Key（custom 另需 baseUrl）后，**自动调用各厂商模型列表接口**拉取可用模型（服务端代理转发，key 不经第三方）：Anthropic `GET /v1/models`、OpenAI `GET /v1/models`、Gemini `GET /v1beta/models`、custom `GET {baseUrl}/models`（OpenAI 兼容）。
- 快速/思考两档模型改为从拉取结果中**下拉选择**（保留手动输入兜底——列表接口不可用或模型未上列表时）。
- 保存前提供**连接测试**：按当前配置发送一条最小消息，返回成功/失败与耗时；测试失败时保存按钮给出警示（可强制保存，因为部分端点消息接口可用但列表接口受限）。
- 失败给差异化提示：401/403 = key 无效或无权限；404 = baseUrl 或模型名错误；超时 = 网络/端点不可达。切换 provider 时清空已拉取列表。claude_cli 模式不适用（无 key，测试改为探测本机 `claude` 命令可用性）。
